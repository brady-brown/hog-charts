...
| Script | Purpose |
|--------|---------|
| `build_onoff_rapm.py` | Core analytics engine. Reconstructs lineups from play-by-play and computes on/off net-rating splits. Outputs `mbb_onoff_{SEASON}_v2.csv`. Also contains the RAPM ridge solve, which is retired and dormant — every pass runs with `fit_rapm=False`, so no `mbb_rapm_*.csv` is written. |
...
| `impact_artifact.py` | Retired. Built `player-impact.json` by joining RAPM ratings with on/off splits. Kept intact for reference, but `build_site.py` no longer imports it and the site no longer ships RAPM. |
