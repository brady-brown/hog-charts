...
def load_counting():
    ...
    reverse = {raw: broad for broad, raws in POS_MAP.items() for raw in raws}
    df["Pos"] = df["_RawPos"].map(reverse).fillna(df["_RawPos"])
    cols = ["Player", "Team", "Conf", "Pos", "GP", "MPG", "PPG", "RPG", "APG",
            "SPG", "BPG", "TOV", "FG%", "eFG%", "3P%", "FT%", "3PM", "3PA",
            "FGM", "FGA", "OREB", "DREB"]
...
def load_rapm():
    ...
    conf = load_conf_lookup()
    df = df.merge(conf, on="Team", how="left")
